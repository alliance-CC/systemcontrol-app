...
| カード送り表示 | 既定はツールを 1 件ずつめくるカード表示（左右ボタン・矢印キー・スワイプ）。`一覧` に切り替えるとシステム単位のカード一覧 |
| 死活監視 | 登録ツールを定期 ping し、🟢🔴🟡⚪ で一覧表示（外部スケジューラから起動） |
| 稼働監視 | `/api/health` で管理システム自体の生存を外部監視 |
| down 通知 | ツールが down / 復旧したときだけ Slack・Discord の Webhook へ通知（`NOTIFY_WEBHOOK_URL`） |
| 権限分離 | `admin`（登録・編集・削除）と `viewer`（閲覧・検索・現場サポートのみ） |
| 質問ログ | 現場の質問を記録し、「資料に当たらなかった質問」を管理者に提示（`/support/logs`・admin のみ） |
